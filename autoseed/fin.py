...
path = "/root/autoseed"
log = open("/root/autoseed/log.txt", 'a')
trigger_list = ['landof.tv','passthepopcorn.me','privatehd.to','morethantv.net','hdts-announce.ru','beyond-hd.me']
...
def check_statue(torrent_id, s):
    if torrent_id not in s:
        return ''
    lft = s.find(torrent_id)
    lft = s.find('State', lft) + 4
    rt = s.find('\n', lft)
    statue = s[lft:rt].strip()
    return statue
...
def main():
    ...
    if len(l) == 1:
        write_log('No same name torrent. Exit.')
        return
    else:
        write_log('Found %d same name torrents. '%len(l))
        for i in l:
